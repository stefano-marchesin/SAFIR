...
flags.add_argument("--model_type", default=0, type=int, help="PV-DBOW: 0; PV-DM: 1.")
...
class Options(object):
	"""options used by doc2vec models"""
...
def main():
	...
	corpus = [TaggedDocument(words=[token for token in doc], tags=[docno]) for docno, doc in tqdm(zip(docnos, docs)) if doc]

	# initialize Doc2Vec/ConceputalDoc2Vec model
	print('initialize model')
	model = Doc2Vec(epochs=opts.epochs, vector_size=opts.embs_size, window=opts.window, alpha=opts.learn_rate, min_alpha=opts.lin_decay, min_count=opts.min_count, 
	...
	epoch_ranker = EpochRanker(corpus, qdict, None, utils, True, opts, model_folder, rankings_folder, qrels_folder)
